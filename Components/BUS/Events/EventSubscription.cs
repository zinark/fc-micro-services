...
public abstract class EventSubscription<T> : IEventSubscription
{
    ...
    {
        if (input == null) throw new ApiException("{0} eventhandler'ina gonderilen input bos olamaz", new
        {
            eventhandler = GetType().FullName
        });

        if (input.GetType() != typeof(T)) throw new ApiException("{0}, Event Handle edilirken beklenen input {1}, fakat {2} gonderildi!", new
        {
            eventhandler = GetType().FullName,
